...
TermCloud.prototype.draw = function(data, options) {
  ...
  var cols = Object.entries(data[0]).length;

  // var valid = (cols >= 2 && cols <= 3 && data.getColumnType(0) == 'string' && data.getColumnType(1) == 'number');
  var valid = (cols >= 2 && cols <= 3 && typeof Object.values(data[0])[0] == 'string' && typeof Object.values(data[0])[1] == 'number');

  if (valid && cols == 3) {
    // valid = data.getColumnType(2) == 'string';
    valid = typeof Object.values(data[0])[2] == 'string';
  }
  ...
  for (var rowInd = 0; rowInd < data.length; rowInd++) {
    ...
    if (f > 0) {
      ...
      var text = data[rowInd]['keyword'];
      // var link = cols == 3 ? data.getValue(rowInd, 2) : null;
      var link = cols == 3 ? data[rowInd]['QuickGO'] : null;
      // var freq = data.getValue(rowInd, 1);
      var freq = data[rowInd]['enrichment'];
